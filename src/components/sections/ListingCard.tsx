...
export default function ListingCard({ listingId }: ListingCardProps) {
  ...
  const [logoUrl] = useDownloadURL(ref(storageBucket, `listings/${listingInfo.data?.name.split(" ").join("")}/${0}`));
  return (
    <Link href="#" className="group relative  rounded-lg p-4 shadow-sm shadow-indigo-100">
      ...
      {listingInfo.data?.uploadedImages ? (
        listingInfo.data.uploadedImages > 0 ? (
